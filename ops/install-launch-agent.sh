#!/bin/bash
# Keep jev running: at login, after a reboot, and after a crash.
#
# Why this exists: jev was started by hand with `open`, so one power cut or
...
# a month.
#
# launchd owns the process after this. That means it will also restart jev if
...
EXEC="$ROOT/build/Jev.app/Contents/MacOS/jevd"
...
# jev starts by itself once the disk is there.
cat > "$PLIST" <<PLIST
...
<dict>
    ...
    <key>StandardOutPath</key><string>$LOGDIR/jev-launchd.out</string>
    <key>StandardErrorPath</key><string>$LOGDIR/jev-launchd.err</string>
</dict>
</plist>
PLIST
...
if launchctl print "gui/$UID/$LABEL" >/dev/null 2>&1; then
  ...
  echo "launchd did not accept the job — see $LOGDIR/jev-launchd.err" >&2
  exit 1
fi
...
Mac reboots while you are away it will sit at the login window with jev not
